...
本任务只处理 PMPFuzz 的工程实验管线：构建、配置、DUT 接入、覆盖率反馈、时间线、数据完整性、baseline 适配、统计聚合和作图。

开始工作前必须完整阅读并严格遵守仓库根目录的 `AGENTS.md`。以下限制不可协商：

1. 不研究、不解释、不复现、不最小化、不分类任何漏洞或安全异常。
2. `nonpass` 只是一个不透明的工程状态。允许计数和记录，但不得读取其内容来判断原因，不得围绕它继续分析。
3. 判断一次运行能否用于覆盖率实验，只依据进程是否完成、case/result/timeline 是否齐全、数据契约是否通过验证。
4. 不得修改 `paper/` 下任何文件。`paper/` 当前是用户已有的未跟踪目录，必须保持原状。
5. Rocket、BOOM、XiangShan 是强制 DUT，不能静默删除。CVA6 必须完成工程接入尝试；只有 readiness 仍然失败、且保存了完整构建与 smoke 日志后，才允许在正式矩阵中标为工程排除。
6. 所有新功能和 bug 修复严格执行 TDD：先写会失败的定向测试并提交，再做最小修复，测试转绿后单独提交。
7. 不覆盖或删除历史实验产物。每次运行必须使用新的、可辨识的输出目录。
8. 不修改服务器上的 `Android`、`work`、`ida-hcli` 目录。
...
只做工程生成和执行，不调用 mismatch 分析、reduction 或漏洞定位流程。
...
不得根据输出内容继续解释 `nonpass`。非零返回码本身也不得自动等同于漏洞或基础设施故障；原样记录，再由产物完整性和观测有效性决定能否贡献事件覆盖率。
...
必须尝试构建和 smoke。如仍失败，保存日志并在 exclusions 中写明工程原因，例如“模拟器未产生所需 probe”或“构建链不可复现”；不得把安全结果作为排除理由。
...
不得研究 baseline 的 mismatch 或失败内容。
...
遇到问题时先修工程问题，不分析 `nonpass`。如果关键门槛未通过，明确写 `NO` 并停止进入 Pilot 或正式长跑，避免浪费服务器时间。
