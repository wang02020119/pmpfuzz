...
from .c910_nonpmp import C910_NONPMP_PROFILES, C910_NONPMP_TARGET
...
def _target_for_cases(cases: list[dict[str, Any]]) -> str:
    explicit_targets = {str(case.get("target") or "") for case in cases if case.get("target")}
    if explicit_targets == {C910_NONPMP_TARGET}:
        return C910_NONPMP_TARGET
    profiles = {str(case.get("profile") or "") for case in cases if case.get("profile")}
    if profiles and profiles <= set(C910_NONPMP_PROFILES):
        return C910_NONPMP_TARGET
