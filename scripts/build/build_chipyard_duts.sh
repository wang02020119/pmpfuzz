...
DEFAULT_VERILATOR_BIN_DIR="${DEFAULT_VERILATOR_BIN_DIR:-$WORKSPACE_DIR/toolchains/eda/bin}"
ROCKET_VERILATOR="${ROCKET_VERILATOR:-$ROOT_DIR/scripts/build/verilator_rocket_wrapper.sh}"
CVA6_VERILATOR_BIN_DIR="${CVA6_VERILATOR_BIN_DIR:-$CHIPYARD_DIR/.conda-env/bin}"
...
build_cva6() {
  ...
  ensure_cva6_pch_aliases
  make CONFIG=CVA6Config VERILATOR_THREADS=1 -j"$JOBS"
}
