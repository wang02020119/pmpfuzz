...
def _load_raw_campaigns(raw_artifact_root: Path) -> list[tuple[Path, dict[str, Any]]]:
    ...
    for metadata_path in sorted(raw_artifact_root.rglob("metrics/campaign_metadata.json")):
        campaign_dir = metadata_path.parent.parent
        meta = _read_json(metadata_path)
        if str(meta.get("coverage_mode") or "") != "bapc":
            continue
        if str(meta.get("method") or "") != "cascade":
            continue
        campaigns.append((campaign_dir, meta))
    if not campaigns:
        raise ValueError(f"no raw BAPC Cascade campaigns found under {raw_artifact_root}")
    return campaigns
...
def _load_raw_bapc_universe(raw_campaign_dir: Path, meta: dict[str, Any]) -> dict[str, Any]:
    rel = str((meta.get("coverage_universe_files") or {}).get("bapc") or "metrics/coverage_universe/bapc_v4.json")
    path = raw_campaign_dir / rel
    if not path.exists():
        path = raw_campaign_dir / "metrics" / "coverage_universe" / "bapc_v4.json"
    if not path.exists():
        raise ValueError(f"missing raw BAPC v4 universe for {raw_campaign_dir}")
    return _read_json(path)


def _build_v4_universe(raw_campaign_dir: Path, meta: dict[str, Any]) -> dict[str, Any]:
    raw_universe = _load_raw_bapc_universe(raw_campaign_dir, meta)
    capabilities = dict(raw_universe.get("capabilities") or {})
    return build_bapc_coverage_universe(
        dut=str(meta.get("dut") or raw_universe.get("dut") or ""),
        generator_seed=int(meta.get("seed") or raw_universe.get("generator_seed") or 0),
...
def _classify_bapc_bins(universe: dict[str, Any], observed_bins: list[str]) -> list[str]:
    ...
    if out_of_contract:
        raise ValueError(f"observed bins outside BAPC-core v4 universe: {out_of_contract[:5]}")
...
def _sidecar_case_index(sidecar_path: Path, sidecar: dict[str, Any]) -> int:
    explicit = sidecar.get("case_index")
    if explicit is not None:
        return int(explicit)
    suffix = sidecar_path.stem.rsplit("_", 1)[-1]
    return int(suffix)
...
def _collect_cascade_observations(raw_campaign_dir: Path, meta: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    sidecars_by_index = {
        _sidecar_case_index(path, _read_json(path)): _read_json(path)
        for path in sorted((raw_campaign_dir / "elfs").glob("*.json"))
    }
    if events:
        cases = sorted(events.items(), key=lambda item: int(item[1].get("completion_seq") or 0))
    else:
        cases = [
            (f"cascade_{meta.get('dut')}_{case_index:04d}", {})
            for case_index in sorted(sidecars_by_index)
        ]
    for case_id, event in cases:
        case_index = _case_index_from_case_id(case_id)
        sidecar = sidecars_by_index.get(case_index)
        if sidecar is None:
            raise ValueError(f"missing cascade sidecar for {case_id}")
        ...
        if isinstance(event.get("bapc_coverage"), dict):
            raw_records = [
                dict(item)
                for item in (event["bapc_coverage"].get("event_records") or [])
        ...
        event_records = runtime_records or raw_records
...
def _build_metadata(
    *,
    ...
    meta["bapc_rate"] = (len(covered_bins) / int(universe["bin_count"])) if universe["bin_count"] else 0.0
    ...
    meta["method"] = "cascade"
    if is_bapc_formal_request(
        coverage_mode=meta.get("coverage_mode"),
        run_class=meta.get("run_class"),
        experiment_protocol_id=meta.get("experiment_protocol_id"),
    ):
        meta["run_class"] = expected_bapc_formal_run_class("cascade") or str(meta.get("run_class") or "")
        meta["experiment_protocol_id"] = BAPC_CONVERGENCE_PROTOCOL_ID
        meta.update(canonical_bapc_formal_runtime(enabled=True))
...
def _write_manifests(
    ...
    *,
    ...
    first = campaigns[0] if campaigns else {}
    if is_bapc_formal_request(
        coverage_mode=first.get("coverage_mode"),
        run_class=first.get("run_class"),
        experiment_protocol_id=first.get("experiment_protocol_id"),
    ):
        _write_json(
            manifests_dir / "experiment-contract.json",
            build_bapc_convergence_contract(
                dut=str(first.get("dut") or ""),
                bin_count=int(universe["bin_count"]),
                bin_set_sha256=str(universe["bin_set_sha256"]),
                variants=sorted({str(item.get("variant") or "") for item in campaigns}),
                seeds=sorted({int(item.get("seed") or 0) for item in campaigns}),
                source_sha=str(first.get("source_sha") or ""),
                source_tree_sha256=str(first.get("source_tree_sha256") or ""),
                dut_sha=str(first.get("dut_sha") or ""),
                dut_binary_sha256=str(first.get("dut_binary_sha256") or ""),
                allowed_source_shas=sorted({str(item.get("source_sha") or "") for item in campaigns if str(item.get("source_sha") or "")}),
                allowed_source_tree_sha256s=sorted(
                    {str(item.get("source_tree_sha256") or "") for item in campaigns if str(item.get("source_tree_sha256") or "")}
                ),
            ),
        )
...
def _rebuild_single_campaign(raw_campaign_dir: Path, raw_meta: dict[str, Any], out_root: Path) -> dict[str, Any]:
    ...
    universe = _build_v4_universe(raw_campaign_dir, raw_meta)
    raw_timeline = [
        json.loads(line)
    ...
    _write_json(universe_dir / "bapc_v4.json", universe)
    ...
    return {"campaign_dir": out_campaign_dir, "metadata": meta, "universe": universe}


def _dut_artifact_root(out_root: Path, dut: str) -> Path:
    return out_root / "dut-roots" / dut


def reanalyze_bapc_v4_cascade_artifact(raw_artifact_root: Path | str, out_root: Path | str) -> dict[str, Path]:
    ...
    for item in rebuilt:
        dut = str(item["metadata"].get("dut") or "")
        by_dut.setdefault(dut, []).append(item)

    for dut, items in by_dut.items():
        experiment_id = str(items[0]["metadata"].get("experiment_id") or "reanalysis")
        artifact_root = _dut_artifact_root(output_root, dut)
        _write_manifests(
            artifact_root,
            experiment_id,
            [item["metadata"] for item in items],
            universe=dict(items[0]["universe"]),
        )
        _write_artifact_hashes(
            artifact_root,
            _artifact_paths(artifact_root),
            artifact_root / "manifests" / "artifact-sha256.txt",
        )
        for item in items:
            report = validate_timeline(Path(item["campaign_dir"]))
            _write_json(Path(item["campaign_dir"]) / "validation.json", report)
        dut_outputs = aggregate(artifact_root, experiment_id)
        outputs.update({f"{dut}:{key}": value for key, value in dut_outputs.items()})
    return outputs


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Reanalyze raw BAPC v4 Cascade artifacts")
    parser.add_argument("--raw-artifact-root", type=Path, required=True)
    parser.add_argument("--out", type=Path, required=True)
    args = parser.parse_args(argv)
    reanalyze_bapc_v4_cascade_artifact(args.raw_artifact_root, args.out)
    return 0
